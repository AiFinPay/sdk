...
- `npm run registry:sync` regenerates `src/splitterRoutes.generated.ts`; `npm run registry:sync:v14 -- --from <evm-contract>` regenerates `src/v14Deployments.generated.ts` from `registry/v14/*.json`; `registry:check` verifies both in CI

## Rules

- Never edit `*.generated.ts` by hand — change `registry/` inputs or `scripts/generate-*.mjs`, then run the matching `registry:sync`.
