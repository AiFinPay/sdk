...
export async function getAgentHistory(options: AgentHistoryOptions): Promise<Record<string, unknown>> {
  ...
  if (options.passport) {
    ...
    identity.wallets = identity.wallets.filter(
      (wallet) => wallet.chain_family === "evm" && Number(wallet.verified_at) > 0
    );
  ...
  if (!address || !/^0x[0-9a-fA-F]{40}$/.test(address))
    throw new Error("History requires an EVM address or a passport with an EVM wallet binding");
  const query = new URLSearchParams({ limit: String(limit), offset: String(offset) });
  if (source === "transactions") query.set("chain", network);
  const response = await fetchImpl(aifinpayApiUrl(base, `/v1/agents/${address.toLowerCase()}/${source}?${query}`), {
    headers: { accept: "application/json" },
  });
  if (!response.ok)
    throw new Error(
      `Payment history unavailable: HTTP ${response.status}; source=${source}. Use receipts for retained prepaid batches; indexed transactions currently cover Polygon only.`
    );
  ...
  const items = (body[source] as Record<string, unknown>[]).map((item) =>
    Object.fromEntries(fields.filter((field) => item[field] !== undefined).map((field) => [field, item[field]]))
  );
  return {
