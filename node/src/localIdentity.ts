...
function seed(value: unknown): string {
  if (typeof value !== "string" || !/^(0x)?[0-9a-fA-F]{64}$/.test(value)) {
    throw new AiFinPayError(
      "SEED_HASH must contain a 32-byte hex seed (64 hex characters); no mnemonic or double hashing"
    );
  }
...
function readJson(path: string): Record<string, unknown> {
  try {
    ...
    /* File contents and parse errors may contain private material. */
  }
...
export function loadLocalWalletIdentity(): LocalWalletIdentity {
  ...
  if (existsSync(projectPath)) {
    ...
    if (matches.length !== 1) {
      throw new AiFinPayError(
        "Select exactly one agents.json record with AIFINPAY_AGENT_ID; refusing an ambiguous wallet"
      );
    }
    return {
      source: "agents.json",
      seedHash: seed(object(matches[0]) ? matches[0].seed_hash : undefined),
    };
  }
  ...
  if (!existsSync(legacyPath)) {
    throw new AiFinPayError(
      "No persistent wallet configured; configure SEED_HASH, aifinpay/agents.json, AIFINPAY_AGENT_SECRET or an existing MCP keystore before loading an agent"
    );
  }
  ...
  if (legacy.enc) {
    if (legacy.enc !== "scrypt-aes-256-gcm" || !env.AIFINPAY_WALLET_PASSPHRASE) {
      throw new AiFinPayError(
        "Encrypted keystore requires AIFINPAY_WALLET_PASSPHRASE and the supported encryption format"
      );
    }
    try {
      ...
      return {
        ...
        secretB58: Buffer.concat([
          decipher.update(Buffer.from(legacy.ct as string, "base64")),
          decipher.final(),
        ]).toString("utf8"),
      };
    } catch {
