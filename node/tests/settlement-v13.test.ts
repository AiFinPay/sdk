...
function executionClients(invoice: NativeSettlementInvoice | StableSettlementInvoice) {
  ...
  const walletClient = {
    account: { address: merchant },
    chain: { id: invoice.chain_id },
    getChainId: vi.fn().mockResolvedValue(invoice.chain_id),
    writeContract,
  };
...
describe("v1.3 contract calldata and confirmation", () => {
  it("refuses an asset mislabeled as USDC and a wallet connected to another chain", async () => {
    ...
    await expect(
      executeSettlementInvoice(
        invoice,
        clients.walletClient as unknown as WalletClient,
        clients.publicClient as unknown as PublicClient,
        pin
      )
    ).rejects.toThrow(/independent token pin/);
    ...
    await expect(
      executeSettlementInvoice(
        native,
        nativeClients.walletClient as unknown as WalletClient,
        nativeClients.publicClient as unknown as PublicClient,
        trustedPin(native)
      )
    ).rejects.toThrow(/wallet chainId/);
    expect(nativeClients.writeContract).not.toHaveBeenCalled();
  });
  it.each(["native", "stable"])(
    "encodes the %s tuple consumed by the contract and waits for success",
    async (asset) => {
      const invoice = asset === "native" ? nativeAifp1() : stableAifp2();
      const clients = executionClients(invoice);
      const result = await executeSettlementInvoice(
        invoice,
        clients.walletClient as unknown as WalletClient,
        clients.publicClient as unknown as PublicClient,
        trustedPin(invoice)
      ...
      expect(decoded.args).toEqual([
        { ...args, grossAmount: BigInt(args.grossAmount), validUntil: BigInt(args.validUntil) },
      ]);
  ...
  it("refuses the obsolete flat signature before any RPC or signature", async () => {
    ...
    (invoice.transaction as { function: string }).function =
      "payNative(bytes32,address,uint256,address,uint256,string)";
    const clients = executionClients(invoice);
    await expect(
      executeSettlementInvoice(
        invoice,
    ...
    ).rejects.toThrow(/function signature/);
...
describe("canonical v1.3 settlement invoice", () => {
  ...
  it.each(["0x10", "1".repeat(79), (2n ** 256n).toString(), "-1"])(
    "rejects an invalid decimal uint256 gross %s",
    (amount) => {
      const invoice = nativeAifp1();
  ...
  it("accepts an independently pinned route only when every route identity field matches", () => {
    ...
    const wrongHash = {
      ...trustedPin(invoice),
      runtime_code_hash: `0x${"55".repeat(32)}` as `0x${string}`,
    };
...
describe("settlement HTTP transport", () => {
  it("uses injected transport, no redirect and the canonical /v1 path", async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      "https://api.aifinpay.io/v1/settlement/routes?route_class=AIFP-1",
