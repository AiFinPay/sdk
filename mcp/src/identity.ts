...
function readJson(path: string): any {
  try {
    ...
    throw new Error(`Cannot read wallet JSON at ${path}; refusing to generate a replacement wallet`);
  }
}
...
export function loadWalletIdentity(config: McpConfig): WalletIdentity | null {
  ...
  try {
    ...
    /* A stat failure does not change which wallet was selected. */
  }
  if (legacy.enc) {
    ...
    try {
      ...
      return {
        ...
        secretB58: Buffer.concat([decipher.update(Buffer.from(legacy.ct, "base64")), decipher.final()]).toString(
          "utf8"
        ),
      };
    } catch {
