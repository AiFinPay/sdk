...
export async function runDevPaymentQuote(ctx: ToolContext, args: Record<string, unknown>) {
  ...
  const version = args.contract_version,
    units = args.units ?? 1000;
  ...
  if (!ctx.config.devMode || !ctx.config.baseUrl)
    return fail("Configure AIFINPAY_MODE=dev and AIFINPAY_BASE_URL explicitly");
  try {
    ...
    if (!challenge?.merchant_id || !challenge.accepted_chains?.includes("amoy"))
      return fail("Dev route has no configured Amoy settlement");
    const quoted = await fetchImpl(`${base}/v1/quote`, {
      method: "POST",
    ...
    if (
      quote.network_mode !== "test" ||
      quote.merchant_id !== challenge.merchant_id ||
      quote.resource !== resource ||
      quote.accepted_chains?.length !== 1 ||
      quote.accepted_chains[0] !== "amoy"
    ) {
      return fail("Quote is not bound to this test merchant/resource on Amoy");
    }
    if (quote.settlement_call?.splitter_version !== version)
      return fail(
        `Requested ${version}; backend advertises ${quote.settlement_call?.splitter_version ?? "no verified version"}. Configure the corresponding dev deployment first.`
      );
    return {
      content: [
        {
          ...
          text: JSON.stringify(
            {
              ...
              next_step:
                "Use a separately verified Amoy executor for this exact deployment; current MCP does not broadcast payments. Keep the quote and transaction reference for receipt recovery.",
            },
            null,
            2
          ),
    ...
    return fail("Dev backend unavailable or returned an invalid response");
  }
}
