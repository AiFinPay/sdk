...
describe("agent_quote AIFP-1 recognition", () => {
  it("reports gross-inclusive AIFP-1 terms without x402 detection", async () => {
    const out = await runAgentQuote(
      ...
      { url: "https://gateway.aifinpay.io/demo/genres" }
    );
    expect(JSON.parse((out as any).content[0].text)).toMatchObject({
      facilitator: "aifp1",
