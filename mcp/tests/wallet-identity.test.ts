...
describe("persistent wallet identity", () => {
  ...
  it("reloads local files within the same connection and preserves the current wallet on an invalid reload", async () => {
    ...
    try {
      ...
      expect(tools).toContain("agent_call");
      expect(tools).toContain("quote_split");
      expect(tools).toContain("payable_fetch");
  ...
  it("init refuses a malformed existing keystore without overwriting it", () => {
    const f = fixture();
    const path = join(f.home, "agent.json");
    const malformed = "{ broken keystore";
    writeFileSync(path, malformed, { mode: 0o600 });
    const bin = fileURLToPath(new URL("../bin/aifinpay-mcp.js", import.meta.url));
    const result = spawnSync(process.execPath, [bin, "init"], {
      encoding: "utf8", timeout: 10000, cwd: f.home,
      env: { PATH: process.env.PATH, AIFINPAY_HOME: f.home },
    });
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("refusing to generate a replacement wallet");
