...
const PASSPORT = {
  agent: {
    agent_id: "aifp_agent_" + "a".repeat(32),
    agent_number: 1,
    agent_number_display: "AIFP-000000001",
    username: "@test_agent",
    status: "active",
    integrity_state: "ok",
...
for (const implementation of ["MCP", "SDK"])
  describe(`${implementation} payment history`, () => {
    async function run(args: any, fetchImpl: typeof fetch) {
      ...
      const result = await runAgentHistory(
        {
          config: { baseUrl: "https://api.aifinpay.io" },
          agent: { evmAddress: ADDRESS, inner: { fetchImpl } },
          log: () => {},
    ...
    it("gets address history with bounded paging and preserves exact amounts", async () => {
      const fetchImpl = (async (url: any) => {
        expect(String(url)).toBe(
          `https://api.aifinpay.io/v1/agents/${ADDRESS}/transactions?limit=2&offset=4&chain=polygon`
        );
        return Response.json({
          transactions: [{ tx_hash: "hash", total_amount: (2n ** 255n).toString() }],
          next_offset: 6,
        });
      }) as typeof fetch;
    ...
    it("refuses address/passport mismatch before fetching history", async () => {
      let calls = 0;
      await expect(
        run({ passport: "AIFP-1", address: "0x" + "cd".repeat(20) }, (async () => {
          calls++;
    ...
    it("does not disguise absent passport support as an empty history", async () => {
      await expect(
        run({ passport: "AIFP-1" }, (async () =>
          Response.json({ error: "not_found" }, { status: 404 })) as typeof fetch)
      ).rejects.toThrow();
    });
    it("strips bearer tokens and unrecognized secret fields from metadata", async () => {
      const result = await run({ address: ADDRESS, source: "receipts" }, (async () =>
        Response.json({
          receipts: [
            {
              receipt_id: "rcpt_one",
              remaining: 2,
              receipt: "bearer-sentinel",
    ...
    it("rejects invalid pagination without making a request", async () => {
      await expect(
        run({ address: ADDRESS, limit: 1000 }, (async () => {
