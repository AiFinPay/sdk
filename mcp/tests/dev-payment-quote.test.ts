...
function context(mode = true, version = "1.4", networkMode = "test") {
  const calls: any[] = [];
  const ctx = {
    config: { devMode: mode, baseUrl: "https://dev.aifinpay.io" },
    agent: {
      ...
      inner: {
        fetchImpl: async (url: string, init?: RequestInit) => {
          ...
          return Response.json({
            ...
            resource: "/v1/dev/paid/data",
            network_mode: networkMode,
            accepted_chains: ["amoy"],
            settlement_call: { splitter_version: version },
          });
        },
      },
    },
...
it("refuses live mode, live quotes and production API origins", async () => {
  ...
  const prod = context();
  prod.ctx.config.baseUrl = "https://api.aifinpay.io";
