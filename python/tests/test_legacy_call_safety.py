...
def test_legacy_call_refuses_paid_challenge_before_broadcast():
    ...
    challenge = {
        ...
        "pay_native": {
            "chain": "polygon",
            "splitter": "0x" + "9" * 40,
            "merchant_wallet": "0x" + "8" * 40,
            "total_wei": "1000",
            "order_id": "evil",
        },
    }
    with (
        patch("aifinpay.unified_agent.requests.get", return_value=response(200, {"providers": [PROVIDER]})),
...
def test_legacy_call_does_not_accept_caller_cost_for_unknown_provider_price():
    ...
    with (
        patch("aifinpay.unified_agent.requests.get", return_value=response(200, {"providers": [unknown_price]})),
        patch("aifinpay.unified_agent.requests.request", return_value=response(402, challenge)) as request,
    ):
