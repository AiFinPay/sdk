...
// both passed the check before either reached the mark. Observed live on the
// sibling Venice bridge: a single transaction authorised two upstream calls,
// and both responses cited it. The atomic primitive already existed and its own
// comment said to use the return value as the guard; the server used it as an
// afterthought.
//
// Claiming before upstream introduces the opposite hazard — a payment taken
// for a service that was never delivered, which is how a Venice order was
// consumed while the provider answered 402 for lack of the bridge's own credit.
// So the claim is a lease that a failure gives back.
//
...
//
// store.js picks its backend at module load, so the flag is set by the npm
// script rather than here — an assignment in this file would run after the
// static import below is hoisted and evaluated, and a top-level `await import`
// to work around that registers the tests too late for the runner to collect
...
test("every upstream call is claimed first", () => {
  ...
  //
  // Counted against the forwardUpstream() dispatch sites, not against the fetch
  // itself: this bridge factors the upstream call into one helper shared by
  // three payment paths — standard x402, Solana and the legacy EVM one. Counting
  // the single fetch would find one call site and pass while two of the three
  // paths reached the provider unguarded, which is exactly the failure this
  // count exists to catch (the first pass at the same fix on the sibling exa
  // bridge guarded one of its two paths and an indexOf() comparison passed).
  const src = readFileSync(new URL("./server.js", import.meta.url), "utf8");
  const upstreamFetches = [...src.matchAll(/await fetch\(UPSTREAM_URL/g)].map((m) => m.index);
  // The lookbehind drops the declaration, which otherwise counts as a fourth
  // call site and inflates the number of claims this test demands.
  const dispatches = [...src.matchAll(/(?<!function )forwardUpstream\(req, res,/g)].map((m) => m.index);
  const claims = [...src.matchAll(/await claimTxLease\(/g)].map((m) => m.index);

  assert.ok(upstreamFetches.length > 0, "no upstream call found — has the bridge changed shape?");
  assert.ok(dispatches.length > 0, "no paid path reaches forwardUpstream — has the bridge changed shape?");
  assert.equal(
    claims.length, dispatches.length,
    `${dispatches.length} paid path(s) but ${claims.length} claim(s) — one path is unguarded`,
  );
  for (const [i, at] of dispatches.entries()) {
    assert.ok(claims[i] < at, `paid path ${i + 1} reaches upstream without a claim`);
  }

  // And every non-delivery path hands the payment back. The releases live in
  // forwardUpstream(), so there is one provider-denied branch per fetch rather
  // than per path.
  assert.ok(src.includes("releaseTxClaim"), "no path releases a claim");
  const providerDenied = [...src.matchAll(/\[401, 402, 403\]/g)].length;
  assert.equal(
    providerDenied, upstreamFetches.length,
