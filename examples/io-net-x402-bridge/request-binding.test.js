...
test("every order-bearing path checks the binding before it spends anything", () => {
  ...
  assert.ok(solGate < src.indexOf("await fetch(IONET_API_URL", solPath), "Solana: binding checked after the upstream call");
  ...
  assert.ok(evmGate < src.indexOf("await fetch(IONET_API_URL", evmPath), "EVM: binding checked after the upstream call");
