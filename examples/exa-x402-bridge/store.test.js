...
// both passed the check before either reached the mark. Observed live: a
// single Venice transaction authorised two upstream calls, and both responses
// cited it. The atomic primitive already existed and its own comment said to
// use the return value as the guard; the servers used it as an afterthought.
//
...
// consumed while the provider answered 402 for lack of its own credit. So the
...
test("every upstream call is claimed first", () => {
  ...
  //
  // Counted rather than checked once: this bridge calls upstream twice, on the
  // EVM and Solana paths, and the first version of the fix guarded only one.
  // A single indexOf() comparison passed while SOL payments stayed replayable.
  const src = readFileSync(new URL("./server.js", import.meta.url), "utf8");
  const upstreamCalls = [...src.matchAll(/await fetch\(EXA_API_URL/g)].map((m) => m.index);
